...
import { 
  ChevronRight, Home, Mail, Car, Calendar, FileText, ClipboardCheck, Layers, Sparkles, FolderOpen
} from "lucide-react";
    ...
    <div className="min-h-screen bg-white flex flex-col overflow-hidden">
      ...
      <main className="flex-1 flex relative h-[calc(100vh-64px)] mt-16">
        ...
        >
          ...
          >
            <button onClick={() => navigate("/dashboard")} className="w-11 h-11 rounded-2xl flex items-center justify-center text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all">
              <Home className="w-5 h-5" />
            </button>
            <button onClick={() => navigate("/my-documents")} className={cn("w-11 h-11 rounded-2xl flex items-center justify-center transition-all", location.pathname === "/my-documents" ? "bg-teal-500 text-white shadow-lg" : "text-slate-400 hover:text-teal-600 hover:bg-teal-50")} title="내 문서함">
