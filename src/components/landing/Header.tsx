          ...
          <div className="flex items-center gap-1 sm:gap-3">
            ...
            {user ? (
              <>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="w-9 h-9 text-muted-foreground hidden sm:flex relative">
                      <Bell className="w-5 h-5" />
                      {totalCount > 0 && (
                        <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] rounded-full bg-destructive text-destructive-foreground text-[10px] font-black flex items-center justify-center px-1 animate-pulse">
                          {totalCount}
                        </span>
                      )}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-80 border border-border shadow-xl max-h-[400px] overflow-y-auto">
                    <DropdownMenuLabel className="flex items-center justify-between">
                      <span className="text-sm font-bold">알림</span>
                      {totalCount > 0 && (
                        <Badge variant="destructive" className="text-[10px] h-5 font-black">
                          {totalCount}건
                        </Badge>
                      )}
                    </DropdownMenuLabel>
                    ...
                    {totalCount === 0 ? (
                      ...
                      <>
                        {myRequestItems.length > 0 && (
                          <>
                            <DropdownMenuLabel className="text-[10px] font-black text-violet-600 uppercase tracking-wider">
                              📋 내가 신청한 건 ({myRequestItems.length})
                            </DropdownMenuLabel>
                            {myRequestItems.slice(0, 3).map((item) => (
                              <DropdownMenuItem
                                key={item.id}
                              ...
                              >
                                ...
                                <span className="text-[11px] text-muted-foreground ml-4">
                                  {item.start_date} ~ {item.end_date} · {item.current_step}
                                </span>
                              </DropdownMenuItem>
                            ))}
                            <DropdownMenuSeparator />
                        ...
                        {myTurnItems.length > 0 && (
                          <>
                            <DropdownMenuLabel className="text-[10px] font-black text-amber-600 uppercase tracking-wider">
                              🔔 내가 승인해야 할 건 ({myTurnItems.length})
                            </DropdownMenuLabel>
                            {myTurnItems.slice(0, 3).map((item) => (
                              <DropdownMenuItem
                                key={item.id}
                                className="cursor-pointer flex flex-col items-start gap-1 py-3"
                                onClick={() => navigate("/leave-request")}
                        ...
                        {myApprovedItems.length > 0 && (
                          <>
                            <DropdownMenuSeparator />
                            <DropdownMenuLabel className="text-[10px] font-black text-blue-600 uppercase tracking-wider">
                              ⏳ 내가 승인 후 진행 중 ({myApprovedItems.length})
                            </DropdownMenuLabel>
                            {myApprovedItems.slice(0, 3).map((item) => (
                              <DropdownMenuItem
                                key={item.id}
                                className="cursor-pointer flex flex-col items-start gap-1 py-3"
                                onClick={() => navigate("/leave-request")}
                    ...
                    {totalCount > 3 && (
                      <>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem className="cursor-pointer text-center justify-center text-primary font-bold text-sm" onClick={() => navigate("/leave-request")}>
                          전체 보기
                        </DropdownMenuItem>
                      </>
                    )}
