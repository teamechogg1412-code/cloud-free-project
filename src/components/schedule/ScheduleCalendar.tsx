...
interface ScheduleEvent {
  ...
  artist?: { id: string; name: string; stage_name: string | null } | null;
  _isLeave?: boolean;
}
...
const TYPE_COLORS: Record<string, string> = {
  ...
  script_reading: "bg-orange-500",
  audition: "bg-violet-500",
  fitting: "bg-rose-500",
  press: "bg-teal-500",
};
    ...
    <div className="bg-white rounded-xl border border-slate-200">
      ...
      <div className={`grid grid-cols-7 ${viewMode === "week" ? "min-h-[200px]" : ""}`}>
        {days.map((day, idx) => {
          ...
          // Separate leave events to show on top
          const leaveEvents = events.filter(e => e.schedule_type === "leave");
          const otherEvents = events.filter(e => e.schedule_type !== "leave");
          ...
          return (
            ...
            >
              ...
              <div className="px-1 pb-1 space-y-0.5">
                {/* Leave events - full width bar style */}
                {leaveEvents.map((ev) => (
                  <div
                    key={ev.id}
                    className="w-full px-1.5 py-0.5 rounded text-[10px] leading-tight bg-emerald-100 text-emerald-700 truncate border border-emerald-200"
                    title={ev.title}
                  >
                    {ev.title}
                  </div>
                ...
                {/* Regular events */}
                {otherEvents.slice(0, viewMode === "week" ? 8 : 3).map((ev) => (
                  <button
                    key={ev.id}
                  ...
                  >
                    ...
                    {ev.artist_id && (ev.artist as any)?.name?.charAt(0)}
                    {ev.artist_id ? " " : ""}
                    {ev.title}
                  </button>
                ))}
                {otherEvents.length > (viewMode === "week" ? 8 : 3) && (
                  <p className="text-[10px] text-slate-400 px-1">
                    +{otherEvents.length - (viewMode === "week" ? 8 : 3)}건
                  </p>
                )}
              </div>
            </div>
