...
import { ExpenseTemplateManager } from "@/components/finance/ExpenseTemplateManager";
...
const FinanceSettings = () => {
  ...
  const [masterConfigs, setMasterConfigs] = useState({
    ...
    OPENBANK_CLIENT_ID: "", OPENBANK_CLIENT_SECRET: "", OPENBANK_BASE_URL: "https://openapi.openbanking.or.kr",
  });
  ...
  const handleGenerateId = async () => {
    ...
    try {
      ...
      const { data, error } = await invokeEdgeFunction("codef-api", { body: bodyPayload });
  ...
  const openAddAccount = (connectedItem?: any, businessType: "BK" | "CD" | "OB" = "BK") => {
    let key = "";
    let type = businessType;
    let org = "";

    if (connectedItem) {
      key = connectedItem.config_key;
      type = key.includes("_BK_") ? "BK" : "CD";
      org = key.split("_").pop()!;
    } else if (businessType === "OB") {
      key = "OPENBANKING";
      org = "OPENBANKING";
    }
  ...
  const handleAddAccount = async () => {
    ...
    setProcessing(true);
    try {
      const connectedIdKey = accountForm.businessType === "OB" ? "OPENBANKING" : accountForm.connectedIdKey;
      const { error } = await supabase.from("finance_accounts").insert({
        tenant_id: currentTenant?.tenant_id,
        connected_id_key: connectedIdKey,
  ...
  const renderOpenBankingSection = () => {
    const obAccounts = accounts.filter((a) => a.business_type === "OB");
    return (
      <Card className="border-none shadow-md rounded-2xl">
        <CardHeader>
          <CardTitle className="text-md flex items-center gap-2 text-indigo-600">
            <Landmark size={18} /> 오픈뱅킹 계좌
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex justify-between">
            <p className="text-sm text-muted-foreground">OpenBanking을 통해 실시간 거래 조회를 지원합니다.</p>
            <Button size="sm" variant="outline" className="text-xs h-7" onClick={() => openAddAccount(undefined, "OB")}>계좌 추가</Button>
          </div>

          {obAccounts.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">등록된 오픈뱅킹 계좌가 없습니다.</p>
          ) : (
            obAccounts.map((acc) => (
              <div key={acc.id} className="flex items-center justify-between text-xs bg-background p-2 rounded-lg">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary" className="text-[10px]">{acc.account_alias || "오픈뱅킹"}</Badge>
                  <span className="font-mono text-muted-foreground">{acc.account_number || "-"}</span>
                </div>
                <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => handleDeleteAccount(acc.id)}>
                  <Trash2 size={12} />
                </Button>
              </div>
            ))
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-28 pb-16 px-6 max-w-6xl mx-auto space-y-8">
        ...
        <Card className="border-none shadow-md rounded-2xl">
          <CardHeader>
            <CardTitle className="text-lg">OPEN BANKING API 설정 (슈퍼어드민 전용)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              OpenBanking 통합 API 키는 시스템 전역 설정으로 슈퍼어드민에서 관리합니다.
            </p>
            <Button variant="outline" onClick={() => window.location.href = "/super-admin/api-management"}>
              슈퍼어드민 API 관리 페이지로 이동
            </Button>
          </CardContent>
        </Card>
