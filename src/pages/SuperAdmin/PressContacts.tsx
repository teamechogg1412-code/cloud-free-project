...
const PressContacts = () => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    const ws = wb.addWorksheet("연락처");
    ws.columns = [
      { header: "언론사", key: "media_company", width: 15 },
      { header: "기자명", key: "reporter_name", width: 10 },
      { header: "이메일", key: "contact_email", width: 25 },
      { header: "연락처", key: "contact_phone", width: 15 },
      { header: "목적", key: "purpose", width: 20 },
    ];
    ws.addRow({ media_company: "조선일보", reporter_name: "홍길동", contact_email: "hong@chosun.com", contact_phone: "010-1234-5678", purpose: "PR (언론 홍보)" });
    ws.addRow({ media_company: "블로터", reporter_name: "김철수", contact_email: "kim@bloter.net", contact_phone: "010-9876-5432", purpose: "" });
    const buffer = await wb.xlsx.writeBuffer();
    const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "미디어_연락처_템플릿.xlsx";
    a.click();
