...
import {
  ...
  ClipboardList, Save, Loader2, Heart, ExternalLink, Car, Camera,
} from "lucide-react";
...
const MyPage = () => {
  ...
  const { user, profile, currentTenant, isSuperAdmin, refreshProfile } = useAuth();
                ...
                {editingName ? (
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      autoFocus
                    />
                    <Button size="sm" onClick={saveName} disabled={savingName || !nameInput.trim()}>
                      {savingName ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => { setEditingName(false); setNameInput(profile?.full_name || ""); }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="w-7 h-7" onClick={() => { setNameInput(profile?.full_name || ""); setEditingName(true); }}>
