        ...
        try {
          ...
          if (tenant) {
            ...
            setFormData({
              ...
              bank_name_en: (tenant as any).bank_name_en || "",
              swift_code: (tenant as any).swift_code || "",
              branch_name_en: (tenant as any).branch_name_en || "",
              bank_address_en: (tenant as any).bank_address_en || "",
              account_number_en: (tenant as any).account_number_en || "",
              account_name_en: (tenant as any).account_name_en || "",
            });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        bank_name_en: formData.bank_name_en || null,
        swift_code: formData.swift_code || null,
        branch_name_en: formData.branch_name_en || null,
        bank_address_en: formData.bank_address_en || null,
        account_number_en: formData.account_number_en || null,
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <Card className="glass-card shadow-md">
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center gap-2 text-lg">
                  <Landmark className="w-5 h-5 text-primary" /> 은행 인보이스 정보 (영문)
                </CardTitle>
                <CardDescription>해외 거래처 인보이스에 사용될 영문 계좌 정보입니다. 은행을 선택하면 본점 정보가 자동 입력됩니다.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {/* 은행 프리셋 선택 */}
                <div className="space-y-2">
                  <Label>은행 선택 (프리셋)</Label>
                  <Select
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="account_name_en">Account Name (예금주 영문)</Label>
                    <Input id="account_name_en" value={formData.account_name_en} onChange={handleChange} placeholder="ARKPORT INC." />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="account_number_en">Account Number</Label>
                    <Input id="account_number_en" value={formData.account_number_en} onChange={handleChange} placeholder="000-000000-00-000" />
                  </div>
                </div>

                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="bank_name_en">Bank Name (영문)</Label>
                    <Input id="bank_name_en" value={formData.bank_name_en} onChange={handleChange} placeholder="Kookmin Bank" className="bg-muted/50" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="swift_code">SWIFT Code</Label>
                    <Input id="swift_code" value={formData.swift_code} onChange={handleChange} placeholder="CZNBKRSE" className="bg-muted/50" />
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input id="branch_name_en" value={formData.branch_name_en} onChange={handleChange} placeholder="Head Office" className="bg-muted/50" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="bank_address_en">Bank Address</Label>
                    <Input id="bank_address_en" value={formData.bank_address_en} onChange={handleChange} placeholder="84, Namdaemun-ro, Jung-gu, Seoul" className="bg-muted/50" />
                  </div>
                </div>
              </CardContent>
            </Card>
