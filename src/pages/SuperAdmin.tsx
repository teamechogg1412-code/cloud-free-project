...
import {
  ...
  MessageSquareShare, Clock, Palmtree, FolderTree, HardDrive, BookOpen, Landmark, FileCode2
} from "lucide-react";
...
const SuperAdmin = () => {
  ...
  const handleApprove = async (request: SignupRequest) => {
    let tenantId = selectedTenants[request.id];
    setProcessingIds((prev) => new Set(prev).add(request.id));
    try {
      // 회사가 선택되지 않았으면 요청 회사명으로 자동 생성
      if (!tenantId) {
        ...
        toast.info(`"${request.company_name}" 회사가 자동 생성되었습니다.`);
      }
      ...
      toast.success("승인 완료");
      fetchData();
    } catch (error: any) { toast.error("오류 발생: " + error.message); } 
      ...
      <main className="pt-24 pb-16 px-4 max-w-[1600px] mx-auto space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            ...
            {/* 표준 규정 관리 */}
            <div 
              onClick={() => navigate("/super-admin/regulations")}
              className="group bg-white p-5 rounded-xl border border-slate-200 hover:border-cyan-500 hover:shadow-md transition-all cursor-pointer flex flex-col gap-3"
            >
              <div className="w-10 h-10 rounded-lg bg-cyan-100 flex items-center justify-center text-cyan-600 group-hover:scale-110 transition-transform">
                <BookOpen className="w-6 h-6" />
              </div>
              <div>
                <h3 className="font-bold text-slate-900">표준 규정 관리</h3>
                <p className="text-xs text-slate-500 mt-1">인사·급여·복무 등 표준 규정 템플릿을 등록하고 관리합니다.</p>
              </div>
            </div>

            {/* 은행 정보 관리 */}
            <div 
              onClick={() => navigate("/super-admin/bank-presets")}
              className="group bg-white p-5 rounded-xl border border-slate-200 hover:border-sky-500 hover:shadow-md transition-all cursor-pointer flex flex-col gap-3"
            >
              <div className="w-10 h-10 rounded-lg bg-sky-100 flex items-center justify-center text-sky-600 group-hover:scale-110 transition-transform">
                <Landmark className="w-6 h-6" />
              </div>
              <div>
                <h3 className="font-bold text-slate-900">은행 정보 관리</h3>
                <p className="text-xs text-slate-500 mt-1">인보이스용 은행 프리셋(SWIFT, 지점 등)을 등록·관리합니다.</p>
              </div>
            </div>

            {/* 시나리오 분석 프롬프트 관리 */}
            <div 
              onClick={() => navigate("/super-admin/scenario-prompts")}
              className="group bg-white p-5 rounded-xl border border-slate-200 hover:border-indigo-500 hover:shadow-md transition-all cursor-pointer flex flex-col gap-3"
            >
              <div className="w-10 h-10 rounded-lg bg-indigo-100 flex items-center justify-center text-indigo-600 group-hover:scale-110 transition-transform">
              ...
              <div>
                <h3 className="font-bold text-slate-900">시나리오 분석 프롬프트</h3>
                <p className="text-xs text-slate-500 mt-1">AI 시나리오 분석 6단계 프롬프트를 등록·편집합니다.</p>
              </div>
            </div>

            {/* 계약서 분석 프롬프트 관리 */}
            <div 
              onClick={() => navigate("/super-admin/contract-prompts")}
              className="group bg-white p-5 rounded-xl border border-slate-200 hover:border-amber-500 hover:shadow-md transition-all cursor-pointer flex flex-col gap-3"
            >
              <div className="w-10 h-10 rounded-lg bg-amber-100 flex items-center justify-center text-amber-600 group-hover:scale-110 transition-transform">
                <FileCode2 className="w-6 h-6" />
              </div>
              <div>
                <h3 className="font-bold text-slate-900">계약서 분석 프롬프트</h3>
                <p className="text-xs text-slate-500 mt-1">AI 계약서 법률 검토 프롬프트를 등록·편집합니다.</p>
              </div>
            </div>

            {/* 보도자료 프롬프트 관리 */}
            <div 
              onClick={() => navigate("/super-admin/press-prompts")}
              className="group bg-white p-5 rounded-xl border border-slate-200 hover:border-rose-500 hover:shadow-md transition-all cursor-pointer flex flex-col gap-3"
            >
              ...
              <div>
                <h3 className="font-bold text-slate-900">보도자료 프롬프트</h3>
                <p className="text-xs text-slate-500 mt-1">AI 기사 생성 8종 프롬프트를 등록·편집합니다.</p>
              </div>
            </div>

            {/* 텔레그램 채널 관리 */}
            <div
              onClick={() => navigate("/super-admin/telegram")}
              className="group bg-white p-5 rounded-xl border border-slate-200 hover:border-cyan-500 hover:shadow-md transition-all cursor-pointer flex flex-col gap-3"
            >
              <div className="w-10 h-10 rounded-lg bg-cyan-100 flex items-center justify-center text-cyan-600 group-hover:scale-110 transition-transform">
                <MessageSquareShare className="w-6 h-6" />
              ...
              <div>
                <h3 className="font-bold text-slate-900">텔레그램</h3>
                <p className="text-xs text-slate-500 mt-1">봇 연동 채널을 고객사와 연결·관리합니다.</p>
              </div>
            </div>
                ...
                <div key={request.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 bg-white p-2 rounded-lg shadow-sm border border-amber-200">
                    <Select onValueChange={(v) => setSelectedTenants((p) => ({ ...p, [request.id]: v }))}>
                      <SelectTrigger className="w-[180px] h-9 text-xs"><SelectValue placeholder="미선택 시 자동생성" /></SelectTrigger>
                      <SelectContent>
